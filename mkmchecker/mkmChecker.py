...
import sheetclient.client as gsclient

from mtgorp.db.load import Loader

APP_PATH = appdirs.AppDirs(
	'mkmchecker',
	'cached',
).user_data_dir

SELLERS_PATH = os.path.join(APP_PATH, 'sellers.bin')

class Requester(object):

	config_path = 'mkmapikeys.cfg'
	...
	def api_request(resource):
		...
		t_params = {
			'oauth_consumer_key': Requester.appToken,
			'oauth_nonce': Requester.get_nonce(),
			'oauth_signature_method': 'HMAC-SHA1',
			'oauth_timestamp': str(int(time.time())),
			'oauth_token': Requester.accessToken,
			'oauth_version': '1.0'
		}

		signature_key = Requester.bAppSecret+b'&'+Requester.bAccessTokenSecret

		uri = 'https://www.mkmapi.eu/ws/v1.1/output.json/' + resource
		params = Requester.url_encode(t_params)

		auth = 'GET&'+urllib.parse.quote_plus(uri)+'&'+params
		signature = base64.b64encode(hmac.new(signature_key, auth.encode('utf-8'), hashlib.sha1).digest())
...
class Product(object):
	...
	def __str__(self):
		return 'Product({0}, {1})'.format(self.name, self.weight)
...
class Article(object):
	...
	@property
	def weight(self):
		return 1/(1+math.e**(-(self.avr_price - self.price)/4))\
...
def load_products():
	...
	print('cardboards loadsd', len(cardboards))
...
def coord_to_string(col, row):
	return str(num_to_col_letters(col))+str(row)
...
def test():
	print(Requester.api_request('products/'+'RishadanPort'+'/1/1/1')['product'])
